...
- In fresh mode, do not copy extracted textbook exercise/question prompts as final questions. Use them only as weak hints about coverage if no better prose is available, then write a fresh teacher-made question from the underlying concept.
- In source_exact mode, stay tightly grounded to selected TXT/PDF concepts. If selected source chunks contain real exercise/question lines, preserve that wording closely with only minimal cleanup or format adaptation.
- In source_exact mode, if selected chunks contain explanation but no real question line, build the question from exact source facts and wording without outside content.
...
In source_exact mode, stay close to selected TXT/PDF wording. Preserve real exercise/question wording when present, with only minimal cleanup or format adaptation.
